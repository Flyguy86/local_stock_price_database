...
echo "🔄 Restarting ray_orchestrator to load new code..."
...
STATUS_RESP=$(echo "$RESPONSE" | python3 -c "import sys, json; print(json.load(sys.stdin).get('status', ''))" 2>/dev/null || echo "")
